...
function App() {
  ...
  const fetchBookmarks = () => {
    fetch('https://bookmark-manager-eaw3.onrender.com/api/bookmarks')
      .then((res) => res.json())
  ...
  // 2. Handle Delete (Fixed Syntax)
  const handleDelete = async (id) => {
    await fetch(`https://bookmark-manager-eaw3.onrender.com/api/bookmarks/${id}`, { 
      method: 'DELETE' 
    });
    fetchBookmarks(); 
  };

  // 3. Handle Form Submission (Fixed Parentheses)
  const handleSubmit = async (e) => {
    e.preventDefault()
    
    const response = await fetch('https://bookmark-manager-eaw3.onrender.com/api/bookmarks', {
      method: 'POST',
